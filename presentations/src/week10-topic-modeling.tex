...
\begin{frame}{Your Turn: Warm-up in English}{Shout out a label for each list --- no wrong answers}
  \vspace{0.3em}
  ...
  \begin{callblue}[What you just did]
    You read six words and inferred a theme. An LDA model does the same thing, just in reverse:
    it reads documents and groups the words that tend to co-occur into lists like these.
  \end{callblue}
\end{frame}

\begin{frame}{Your Turn: Now in Korean}{Same game --- each list is the top six words of one topic}
  \vspace{0.3em}
...
\begin{frame}{Your Turn: Answers}{Plausible labels for the Korean lists}
  \vspace{0.1em}
  \begin{center}
    \small
    \begin{tabular}{@{}c l l@{}}
...
\begin{frame}{What the Widget Actually Eats}{Bag-of-word counts, built internally --- no separate widget needed}
  LDA works on a \textbf{term--document matrix of word counts}: rows are documents,
  columns are words, each cell is how many times that word appears in that document.
  That is a \textbf{bag of words}.

  \vspace{0.5em}
  \begin{itemize}
    \item You do \emph{not} add a \texttt{Bag of Words} widget to the pipeline.
      Orange's \texttt{Topic Modelling} widget builds the count matrix internally from your preprocessed tokens.
    \item You do \emph{not} pass TF--IDF weights into LDA. The algorithm models word \emph{counts}
      (it treats a document as if words were drawn from topic urns), so raw frequencies are what it expects.
    \item Preprocessing is what shapes the bag: stopword removal, POS filter, frequency trimming.
      Garbage in, garbage out.
  \end{itemize}

  \vspace{0.4em}
  \begin{callblue}[Where this fits in Weeks 4--5]
    BoW and TF--IDF are two ways of representing a corpus. BoW is \emph{counts}; TF--IDF is \emph{counts reweighted} to emphasise distinctive words.
    LDA uses BoW; earlier in the term we used TF--IDF for search-like tasks and clustering.
  \end{callblue}
\end{frame}
