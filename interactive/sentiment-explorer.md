...
(function () {
  ...
  function showCorpus() {
    ...
    html += '<details class="code-ribbon"><summary><span class="ribbon-label">Show R code: load and explore the corpus</span><span class="ribbon-tag">R</span></summary><div class="code-ribbon-body">';
  ...
  function showScoring() {
    ...
    html += '<div class="callout callout-warn">These scores use raw substring matching against our dictionaries &mdash; no morphological analysis. In Orange, you\'ll preprocess first with Kiwi, which improves accuracy.</div>';
    html += '<details class="code-ribbon"><summary><span class="ribbon-label">Show R code: load dictionaries and score tweets</span><span class="ribbon-tag">R</span></summary><div class="code-ribbon-body">';
    html += '<div class="code-block"><div class="code-block-header"><span>R</span><button class="copy-btn" onclick="copyCode(this)">Copy</button></div>';
    html += '<pre><code><span class="r-comment"># Load sentiment dictionaries</span>\npos_words <span class="r-operator">&lt;-</span> <span class="r-function">read_lines</span>(<span class="r-string">"positive.txt"</span>) <span class="r-operator">|&gt;</span>\n  <span class="r-function">str_subset</span>(<span class="r-string">"[\\\\uAC00-\\\\uD7AF]"</span>) <span class="r-operator">|&gt;</span>  <span class="r-comment"># keep Korean entries only</span>\n  <span class="r-function">str_trim</span>()\nneg_words <span class="r-operator">&lt;-</span> <span class="r-function">read_lines</span>(<span class="r-string">"negative.txt"</span>) <span class="r-operator">|&gt;</span>\n  <span class="r-function">str_subset</span>(<span class="r-string">"[\\\\uAC00-\\\\uD7AF]"</span>) <span class="r-operator">|&gt;</span>\n  <span class="r-function">str_trim</span>()\n\n<span class="r-comment"># Score one tweet: count dictionary matches</span>\nscore_tweet <span class="r-operator">&lt;-</span> <span class="r-keyword">function</span>(text, pos_dict, neg_dict) {\n  pos_hits <span class="r-operator">&lt;-</span> <span class="r-function">sum</span>(<span class="r-function">str_detect</span>(text, <span class="r-function">fixed</span>(pos_dict)))\n  neg_hits <span class="r-operator">&lt;-</span> <span class="r-function">sum</span>(<span class="r-function">str_detect</span>(text, <span class="r-function">fixed</span>(neg_dict)))\n  <span class="r-function">tibble</span>(pos_count <span class="r-operator">=</span> pos_hits, neg_count <span class="r-operator">=</span> neg_hits,\n         score <span class="r-operator">=</span> pos_hits <span class="r-operator">-</span> neg_hits)\n}\n\n<span class="r-comment"># Score all tweets</span>\nscored <span class="r-operator">&lt;-</span> tweets <span class="r-operator">|&gt;</span>\n  <span class="r-function">filter</span>(<span class="r-operator">!</span><span class="r-function">is.na</span>(text)) <span class="r-operator">|&gt;</span>\n  <span class="r-function">mutate</span>(sent <span class="r-operator">=</span> <span class="r-function">map</span>(text, score_tweet, pos_words, neg_words)) <span class="r-operator">|&gt;</span>\n  <span class="r-function">unnest</span>(sent)</code></pre></div>';
    html += '<div class="callout callout-tip"><strong>In Orange:</strong> this is what the <strong>Score Documents</strong> widget does when you load <code>positive.txt</code> and <code>negative.txt</code> as custom word lists.</div>';
    html += '</div></details>';
    html += '</div>';
    detailPanel.innerHTML = html;
  ...
  function showExplore() {
    ...
    html += '<div class="callout callout-warn">Notice false positives in the negative list: <strong>\uC800\uB294</strong> (I + topic marker) and <strong>\uC9C0\uC9C0</strong> (support, not lose) are matched as negative because they appear as substrings in the dictionary. This is why preprocessing and domain-specific dictionaries matter!</div>';
    ...
    html += '<div class="code-block"><div class="code-block-header"><span>R</span><button class="copy-btn" onclick="copyCode(this)">Copy</button></div>';
    html += '<pre><code><span class="r-comment"># Find which positive words match most often</span>\npos_match_counts <span class="r-operator">&lt;-</span> <span class="r-function">map_dfr</span>(pos_words, <span class="r-keyword">function</span>(w) {\n  n <span class="r-operator">&lt;-</span> <span class="r-function">sum</span>(<span class="r-function">str_detect</span>(scored<span class="r-operator">$</span>text, <span class="r-function">fixed</span>(w)))\n  <span class="r-function">tibble</span>(word <span class="r-operator">=</span> w, count <span class="r-operator">=</span> n)\n}) <span class="r-operator">|&gt;</span>\n  <span class="r-function">filter</span>(count <span class="r-operator">&gt;</span> <span class="r-number">0</span>) <span class="r-operator">|&gt;</span>\n  <span class="r-function">arrange</span>(<span class="r-function">desc</span>(count))\n\npos_match_counts <span class="r-operator">|&gt;</span> <span class="r-function">head</span>(<span class="r-number">15</span>)\n\n<span class="r-comment"># Most positive tweets</span>\nscored <span class="r-operator">|&gt;</span>\n  <span class="r-function">arrange</span>(<span class="r-function">desc</span>(score)) <span class="r-operator">|&gt;</span>\n  <span class="r-function">select</span>(tweet_date, period3, score, text) <span class="r-operator">|&gt;</span>\n  <span class="r-function">head</span>(<span class="r-number">5</span>)\n\n<span class="r-comment"># Most negative tweets</span>\nscored <span class="r-operator">|&gt;</span>\n  <span class="r-function">arrange</span>(score) <span class="r-operator">|&gt;</span>\n  <span class="r-function">select</span>(tweet_date, period3, score, text) <span class="r-operator">|&gt;</span>\n  <span class="r-function">head</span>(<span class="r-number">5</span>)</code></pre></div>';
