"""
Korean Text Preprocessing for Orange Data Mining (Mac) — Hanja-aware variant.

For Hanmun-mixed corpora (Colonial Magazines, Kaebyok, older newspaper articles)
where Chinese characters appear inline with Hangul. The script:

  1. Converts every Hanja in the text to its Hangul reading (`hanja` package).
  2. Tokenizes the resulting all-Hangul text with Kiwi (`kiwipiepy`).
  3. Applies the same POS / stopword / doc-frequency filters as the standard
     `custom_preprocessing_mac-users.py`.

Output: a `processed_text` column on the corpus, ready for Bag of Words /
Topic Modelling / Distances / Sentiment Analysis downstream.
...
MAX_DOC_FREQ = 0.99   # Maximum proportion of documents a word can appear in (99%)
...
def hanja_to_hangul(text):
    """Replace every Hanja character with its Hangul reading."""
    if pd.isna(text):
        return ""
    # 'substitution' mode: 韓國 → 한국 (default reading)
    return hanja.translate(str(text), 'substitution')
...
def clean_text(text):
    ...
    # Keep Hangul (precomposed and jamo) + Hanja (CJK Unified) + ASCII word chars + spaces
    text = re.sub(r'[^\w\sㄱ-ㅣ가-힣ᄀ-ᇿ一-鿿]', ' ', text)
...
def preprocess(text):
    ...
    # Hanja → Hangul, then Kiwi tokenize.
...
print(f"✓ Processed {len(processed)} documents (with Hanja → Hangul conversion)")
